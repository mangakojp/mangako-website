        ...
        <div className="bg-mangako-ivory min-h-screen">
                ...
                <div className="max-w-[1400px] mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
                    ...
                    <div className="relative group overflow-hidden h-[900px] flex items-center justify-center p-8 bg-white border border-mangako-ink/10 shadow-2xl">
                        ...
                        <div className="relative z-10 w-full h-full flex items-center justify-center grayscale group-hover:grayscale-0 transition-all duration-[2s]">
                            ...
                            <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center text-center">
                                <h2 className="font-mincho text-2xl font-black text-mangako-ink tracking-widest leading-tight uppercase group-hover:scale-110 transition-transform origin-center">{detailData.title}</h2>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-16 lg:pl-16">
                        <div className="flex flex-col gap-8">
                            <span className="font-sans text-xs tracking-[0.4em] font-black text-mangako-coral uppercase border-b border-mangako-coral/20 pb-2 self-start">{detailData.series}</span>
                            <h1 className="font-mincho font-black text-5xl lg:text-7xl text-mangako-ink tracking-widest leading-tight">{detailData.title}</h1>
                        ...
                        <div className="flex flex-col gap-12">
                            <SectionHeader
                                title={detailData.provenanceTitle}
                                subtitle={detailData.provenanceSubtitle}
                                className="mb-0"
                            />
                        ...
                        <button className="bg-mangako-ink text-mangako-ivory px-16 py-8 text-xs font-black tracking-[0.5em] font-sans uppercase hover:bg-mangako-coral transition-all rounded-full shadow-2cl shadow-mangako-ink/30 transform hover:-translate-y-1 duration-500 mt-12 self-start">
                            {detailData.ctaInquire}
                        </button>
                    </div>
                </div>
            </section>
            ...
            <section className="py-24 bg-mangako-cream/10 border-t border-mangako-ink/5">
                <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
                    <SectionHeader
                        title={detailData.relatedTitle}
                        subtitle={detailData.relatedSubtitle}
                        className="mb-20 text-center"
                    />
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                        {detailData.relatedItems.map((item: any, idx: number) => (
                            <ArchiveCard key={idx} {...item} />
                        ))}
                    </div>
                </div>
            </section>
