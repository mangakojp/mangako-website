...
export const MarketplacePage = () => {
    ...
    return (
        <div className="bg-mangako-ivory min-h-screen">
            <PageHero
                title={marketData?.heroTitle || "Marketplace"}
                subtitle={marketData?.heroSubtitle || "真正性のマーケット。"}
                description={marketData?.heroDesc || "検証済みの希少なマンガ・コレクションが、新たな所有者のもとへ。"}
            />
            ...
            <section className="py-24 relative overflow-hidden">
                <div className="max-w-[1400px] mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
                    <div className="order-2 lg:order-1 relative z-10">
                        <SectionHeader
                            title={marketData?.featuredTitle || "Featured Selection"}
                            subtitle={marketData?.featuredSubtitle || "注目。"}
                        />
                        <Link to={`/marketplace/${marketData?.featuredItemSlug || "featured-item"}`} className="block">
                            <div className="bg-white border border-mangako-ink/10 p-12 hover:shadow-2xl transition-all duration-700 relative overflow-hidden group">
                                ...
                                <h3 className="font-mincho text-3xl font-black text-mangako-ink mb-6 tracking-widest">{marketData?.featuredItemTitle || "Featured Item"}</h3>
                                <p className="font-sans text-lg text-mangako-ink/60 font-light mb-12">{marketData?.featuredItemDesc || "真正性と価値が保証された、至高のコレクションピース。"}</p>
                    ...
                    <div className="order-1 lg:order-2 h-[700px] bg-white relative p-8 group overflow-hidden border border-mangako-ink/10 shadow-2xl">
                        ...
                        <div className="relative z-10 w-full h-full border border-mangako-ink/5 flex items-center justify-center overflow-hidden">
                            {marketData?.featuredItemImage ? (
                                <img
                                    src={marketData.featuredItemImage}
                                    alt={marketData?.featuredItemTitle}
                                    className="w-full h-full object-contain grayscale-[0.2] group-hover:grayscale-0 transition-all duration-[2s]"
                                />
                            ) : (
                                <>
            ...
            <section className="py-24 bg-mangako-cream/30">
                <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
                    <div className="flex flex-col md:flex-row items-end justify-between mb-20 gap-8 border-b border-mangako-ink/10 pb-12">
                        <SectionHeader
                            title={marketData?.latestTitle || "Latest Listings"}
                            subtitle={marketData?.latestSubtitle || "最新の出品。"}
                            className="mb-12"
                        />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {(marketData?.listings || []).map((item: any, idx: number) => (
                            <Link key={idx} to={`/marketplace/${item.id || idx}`} className="block">
                                <ListingCard
                                    {...item}
                                />
                            </Link>
