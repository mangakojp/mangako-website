...
export const MarketplaceDetailPage = () => {
    ...
    // In a real app we'd fetch by slug, here we assume all slugs show the featured detail if it matches
    const isFeatured = slug === "featured-item" || slug === (marketData?.featuredItemSlug);
    const displayData = isFeatured ? marketDetailData : marketDetailData; // Simplified fallback

    return (
        <div className="bg-mangako-ivory min-h-screen">
            <div className="max-w-[1400px] mx-auto px-6 lg:px-12 pt-32">
                <Link to="/marketplace" className="inline-flex items-center gap-4 group">
                    ...
                    <span className="font-sans text-[10px] tracking-[0.5em] font-black text-mangako-ink/40 uppercase group-hover:text-mangako-coral transition-colors">Back to Market</span>
                </Link>
            </div>
            ...
            <section className="pt-24 pb-32">
                <div className="max-w-[1400px] mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
                    ...
                    <div className="relative h-[900px] bg-white border border-mangako-ink/10 shadow-2xl p-12 overflow-hidden group">
                        ...
                        <div className="relative z-10 w-full h-full border border-mangako-ink/5 flex flex-col items-center justify-center p-8 text-center transition-colors duration-[1.5s] overflow-hidden">
                            {displayData?.image ? (
                                <img
                                    src={displayData.image}
                                    alt={displayData.title}
                                    className="w-full h-full object-contain grayscale-[0.3] group-hover:grayscale-0 transition-all duration-[2s]"
                                />
                            ) : (
                                <div className="bg-mangako-ivory/20 backdrop-blur-3xl w-full h-full flex flex-col items-center justify-center">
                                    ...
                                    <div className="mt-20 flex flex-col items-center gap-6">
                                        <h2 className="font-mincho text-3xl font-black text-mangako-ink tracking-widest leading-loose italic">{displayData?.title || "Market Item"}</h2>
                    ...
                    <div className="flex flex-col gap-12 lg:pl-16">
                        <div className="flex flex-col gap-8">
                            <div className="flex items-center gap-6">
                                ...
                                <span className="font-sans text-xs tracking-[0.4em] font-black text-mangako-ink/40 uppercase">Ref: {marketDetailData?.ref || "REF-000"}</span>
                            </div>
                            <h1 className="font-mincho font-black text-5xl lg:text-7xl text-mangako-ink tracking-widest leading-tight italic">{marketDetailData?.title || "Untitled Listing"}</h1>
                        ...
                        <div className="flex flex-col gap-8 bg-mangako-cream/10 p-12 border border-mangako-ink/5 relative group">
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <h4 className="font-mincho text-xl font-bold text-mangako-ink tracking-widest">{marketDetailData?.verificationTitle || "Verification"}</h4>
                            </div>
                            <p className="font-sans text-sm text-mangako-ink/50 font-light leading-relaxed mb-8">{marketDetailData?.verificationDesc || "Mangakoの真正性検証チームが、すべての出品物を個別に鑑定しています。"}</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12">
                            <button className="bg-mangako-ink text-mangako-ivory px-12 py-7 text-xs font-black tracking-[0.5em] font-sans uppercase hover:bg-mangako-coral transition-all rounded-full shadow-2xl shadow-mangako-ink/30 transform hover:-translate-y-2 duration-500">
                                {marketDetailData?.ctaInquire || "Inquire"}
                            </button>
                            <button className="bg-white border border-mangako-ink/10 text-mangako-ink px-12 py-7 text-xs font-black tracking-[0.5em] font-sans uppercase hover:border-mangako-coral transition-all rounded-full shadow-xl shadow-mangako-ink/5 transform hover:-translate-y-2 duration-500">
                                {marketDetailData?.ctaReserve || "Reserve"}
                            </button>
                        </div>
                    </div>
                </div>
