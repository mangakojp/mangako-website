...
export const CommunityPage = () => {
    ...
    return (
        <div className="bg-mangako-ivory min-h-screen">
            <PageHero
                title={communityData?.heroTitle || "Collector Circle"}
                subtitle={communityData?.heroSubtitle || "閉じた輪、開かれた価値。"}
                description={communityData?.heroDesc || "Mangakoのコミュニティは、真の価値を理解するコレクターたちが集い、知識と情熱を分かち合う場所です。"}
            />
            ...
            <section className="py-48 relative overflow-hidden bg-mangako-ink text-mangako-ivory">
                ...
                <div className="max-w-[1200px] mx-auto px-6 relative z-10 text-center">
                    ...
                    >
                        ...
                        <SectionHeader
                            title={communityData?.accessTitle || "Private Membership"}
                            subtitle={communityData?.accessSubtitle || "会員制コミュニティについて。"}
                            className="mb-16 text-center"
                        />

                        <p className="font-mincho text-3xl lg:text-5xl text-mangako-ivory tracking-widest leading-tight mb-20 italic">
                            {communityData?.membershipQuote || "価値を語ることは、文化を語ることに等しい。"}
                        </p>
            ...
            <section className="py-48 bg-white relative overflow-hidden">
                <div className="max-w-[1000px] mx-auto px-6 text-center flex flex-col items-center gap-12 relative z-10">
                    ...
                    <SectionHeader
                        title={communityData?.joinTitle || "Join the Circle"}
                        subtitle={communityData?.joinSubtitle || "入会案内。"}
                        className="mb-0"
                    />
                    <p className="font-sans text-xl text-mangako-ink/50 font-light max-w-2xl text-center leading-relaxed">
                        {communityData?.joinDesc || "Mangakoのコミュニティには、審査制の入会申請が必要です。"}
                    </p>
                    <Link to="/early-access" className="bg-mangako-ink text-mangako-ivory px-16 py-8 text-xs font-black tracking-[0.5em] font-sans uppercase hover:bg-mangako-ink/80 transition-all rounded-full shadow-2xl shadow-mangako-ink/30 transform hover:scale-110 active:scale-95 duration-500 mt-12 inline-block text-center">
                        {communityData?.requestAccess || "Request Invitation"}
                    </Link>
