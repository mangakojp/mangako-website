...
export const ArchivePage = () => {
    ...
    return (
        <div className="bg-mangako-ivory min-h-screen">
            <PageHero
                title={archiveData.heroTitle}
                subtitle={archiveData.heroSubtitle}
                description={archiveData.heroDesc}
            />
                ...
                <div className="max-w-[1400px] mx-auto px-6 lg:px-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-16 relative z-10">
                    {archiveData.entries.map((entry: any, idx: number) => (
                        <motion.div
                            ...
                            className="bg-white border border-mangako-ink/10 p-12 hover:shadow-2xl transition-all duration-700 cursor-pointer group"
                        >
                            ...
                            <button className="font-sans text-[10px] tracking-[0.5em] font-black text-mangako-ink uppercase border-b border-mangako-ink/40 group-hover:border-mangako-coral group-hover:text-mangako-coral pb-1 transition-all">
                                {archiveData.exploreLink}
                            </button>
                        </motion.div>
                    ))}
                </div>
